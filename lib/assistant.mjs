...
Preferências de estilo definidas pelo operador (não substituem as regras acima): ${s.instructions || 'Tom amigável, sem exagero em emojis.'}`;
  ...
  context(id, scope) {
    ...
    const w = this.store.workspace(scope), memories = this.store.data.settings.memoryEnabled && !thread.contextOnly ? w.memories : [];
  ...
  async run(id, scope, job) {
    ...
    try {
      ...
      const readOnly = !!this.store.get(id,scope).contextOnly;
      let text = '';
      for (let step = 0; step < 4; step++) {
        ...
        const tools = step < 3 && !readOnly ? [tool, ...(this.store.data.settings.webSearch ? [{ type: 'web_search' }] : [])] : [];
        const result = await this.provider({ model: scope === 'owner' ? this.model : this.dmModel,
          instructions: this.instructions(scope) + (readOnly ? '\nEsta consulta contém um chat externo. Apenas analise o conteúdo ou redija a resposta solicitada. Ferramentas e memórias privadas não estão disponíveis nesta consulta.' : ''), input, tools, parallel_tool_calls: false,
        ...
        if (readOnly) deny(403,'Uma consulta de chat não pode executar ferramentas.');
        input.push(...output);
